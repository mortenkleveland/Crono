...
- (id)initWithFrame:(CGRect)frame
{
	if ((self = [super initWithFrame:frame]))
	{
        ...
        //self.backgroundImage = [UIImage imageNamed:@"knobMedium"];
        [self setBackgroundImage:[UIImage imageNamed:@"knobMinimalistic.png"]];
        [self setKnobImage:[UIImage imageNamed:@"knobMinimalisticTop"] forState:UIControlStateNormal];
	}
	return self;
}
...
- (void)setKnobImage:(UIImage *)image forState:(UIControlState)theState
{
	if (theState == UIControlStateNormal)
	{
		if (image != _knobImageNormal)
		{
			...
			{
                float mlt = 1.2;
                _knobImageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.origin.x - 7.5, self.bounds.origin.y + 1, self.bounds.size.width*mlt, self.bounds.size.height*mlt)];
                [self addSubview:_knobImageView];
                [self sendSubviewToBack:_knobImageView];
