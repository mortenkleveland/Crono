...
- (id)initWithFrame:(CGRect)frame withOctaveCount:(int)count {
  if (self = [super initWithFrame:frame]) {    
    ...
    keyDownSet = [NSMutableSet setWithCapacity:0];
...
//    float yPos = 1 - (position.y / touch.view.frame.size.height);
//    if (yPos > 1) yPos = 1;
//    else if (yPos < 0) yPos = 0;
//    self.yPosition = [NSNumber numberWithFloat:yPos];
//    [[NSNotificationCenter defaultCenter]postNotificationName:@"yPos" object:nil];
    //NSLog(@"%f", yPos);
}
...
- (float)getYValue:(UITouch*)touch
{
    CGPoint yPos = [touch locationInView:touch.view];
    return yPos.y;
